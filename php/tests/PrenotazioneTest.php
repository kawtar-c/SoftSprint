...
class PrenotazioneTest extends TestCase
{
    ...
    public function testModificaPrenotazione()
    {
        ...
        // Query aggiornata per includere id_tavolo come nella classe originale
        $dbMock->expects($this->once())
               ->method('prepare')
               ->with("UPDATE prenotazione SET nome = ?, telefono = ?, data = ?, persone = ?, fascia_oraria = ?, id_tavolo = ? WHERE id_prenotazione = ?;")
               ->willReturn($stmtMock);

        // bind_param ora si aspetta 7 parametri (sssisii)
        $stmtMock->expects($this->once())
                 ->method('bind_param')
                 ->with(
                     "sssisii",
        ...
        // Chiamata con 7 argomenti per evitare l'ArgumentCountError

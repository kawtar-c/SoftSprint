...
// Controllo ruolo cameriere
if (!isset($_SESSION['user_id']) || $_SESSION['ruolo'] !== 'cameriere') {
    header("Location: login.php");
    exit;
}

// Recupero tavolo selezionato da GET
...
// Piatti già ordinati per questo tavolo
$ordine = new Ordine();
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Aggiorna titolo
    const titolo = document.getElementById("ordine-titolo");
    titolo.textContent = "Ordine Tavolo " + (tavolo || "");

    function aggiornaTotale(){
        let t = 0;
        list.querySelectorAll(".order-item").forEach(el=>{
            t += parseFloat(el.dataset.price) * parseInt(el.querySelector(".qty").textContent);
        });
    ...
    document.querySelectorAll(".dish-add").forEach(btn=>{
        btn.onclick = ()=>{
            ...
            if(!id){ alert("Errore: id piatto non valido"); return; }
            const price = parseFloat(d.dataset.price) || 0;
            const title = d.dataset.title || "Sconosciuto";

            let exist = [...list.children].find(el=>el.dataset.id === id);
            if(exist){
                exist.querySelector(".qty").textContent = parseInt(exist.querySelector(".qty").textContent) + 1;
            } else {
                ...
                li.innerHTML = `
                    ...
                    <strong class="order-item-price">${price.toFixed(2).replace(".", ",")} €</strong>
                `;
                list.appendChild(li);
            }
            aggiornaTotale();
    ...
    // Delegazione +/- 
    list.onclick = e=>{
        if (!e.target.classList.contains("qty-plus") && !e.target.classList.contains("qty-minus")) return;
        ...
        if(qty <=0) li.remove(); else li.querySelector(".qty").textContent = qty;
        aggiornaTotale();
    };
    ...
    document.getElementById("btn-invia").onclick = ()=>{
        ...
        const piatti = [...list.children].map(el=>({id: el.dataset.id, qty: parseInt(el.querySelector(".qty").textContent)}));
        if(!piatti.length){ alert("Aggiungi almeno un piatto!"); return; }

        fetch("../php/config/salva_ordine.php", {
            method:"POST",
            headers:{"Content-Type":"application/json"},
            body: JSON.stringify({tavolo, note: document.getElementById("note-cucina").value, piatti})
        })
        .then(r=>r.json())
        .then(res=>{
            if(res.success){ alert("Ordine inviato!"); location.reload(); }
...
<script>
    // Memorizza ordini già notificati
